...
router.get('/:name/:playlistDesc/:numTracks/:market/:makePublic/:id/:main/:weatherDesc/:temp/:city/:clouds/:user/:accessToken', function (req, res) {
  var _req$params = req.params,
      ...
      accessToken = _req$params.accessToken;
  console.log(playlistDesc == ""); // assign playlist properties, use default if unchanged or blank
  ...
  console.log("custom playlist:", playlist); // get track parameters

  var track = generateTrackParameters(id, clouds, temp);
  var getTracksUrl = "https://api.spotify.com/v1/recommendations?" + "limit=".concat(playlist.limit) + "&seed_genres=".concat(track.seed_genre) + "&target_acousticness=".concat(track.acousticness) + "&target_danceability=".concat(track.danceability) + "&target_energy=".concat(track.energy) + "&target_valence=".concat(track.valence);
  ...
  request.get(getTracks, function (trackError, trackResponse, trackBody) {
    ...
    if (!trackError && trackResponse.statusCode == 200) {
      ...
      res.send("Error: Country (".concat(playlist.market, ") doesn't have an existing market on Spotify. Please try again without using that market."));
    }
  });
...
function generateTrackParameters(id, clouds, temp) {
  ...
  var weatherGroup = Math.floor(id / 100 % 10);
  console.log("id: ", id);
  console.log("weather group: ", weatherGroup);
  console.log("clouds: ", clouds);
  console.log("cloudiness: ", cloudWeight);
  console.log("temp: ", temp);
  console.log("tempWeight: ", tempWeight); // SET SEED GENRE
  // clear skies
  ...
  console.log("weather type: ", weather); // SET ACOUSTICNESS => cloudier = more acoustic
